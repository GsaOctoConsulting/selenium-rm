...
public class RoleAndRoleHistoryStep {
	...
	public void _2rh_user_logs_in_workspace_with_dra_role() throws Throwable {
		SignInUtility.signIntoWorkspace(ConstantsAccounts.DEPT_ROLEADMIN_2, Constants.USERPASS,
				ConstantsAccounts.DEPT_ROLEADMIN_2_SECRETKEY, Constants.USER_FED);
	}

	@And("^_2rh dra navigates to user directory and looks up a user with contracting specialist role$")
	public void _2rh_dra_navigates_to_user_directory_and_looks_up_a_user_with_contracting_specialist_role()
			throws Throwable {
		LaunchBrowserUtil.scrollAllTheWayDown();
		UserDirectoryWidgetUtility.clickUserDirectoryLink();
		UserDirectoryPage.searchUserInUserPicker(ConstantsAccounts.CONTRACT_DATA_CONTRACTINGSPECIALIST_1);
	...
	@When("^_2rh dra adds assiting user role on assistance listing to this user$")
	public void _2rh_dra_removes_the_role_for_the_user() throws Throwable {
		AssignRolePage.selectOrgIfFound(Constants.ORG_GSA, 1);
	...
	@Then("^_2rh the role history should update in the profile for dra$")
	public void _2rh_the_role_history_should_update_in_the_profile_for_dra() throws Throwable {
		// --------------------------
		boolean roleHistoryFound = UserDirectoryViewAccessPage.latestRoleHistoryFound("shah raiaan",
				Constants.ROLEHISTORY_STATUS_ROLE_UPDATED, Constants.ROLE_ASSISTANCE_USER, Constants.ORG_GSA,
				Constants.GO_INTO_ROLE_UPDATED);
		Assert.assertEquals(true, roleHistoryFound);
	...
	@And("^_2rh the role history should update in the profile for the user$")
	public void _2rh_the_role_history_should_update_in_the_profile_for_the_user() throws Throwable {
		SignInUtility.signIntoWorkspace(ConstantsAccounts.CONTRACT_DATA_CONTRACTINGSPECIALIST_1, Constants.USERPASS,
				ConstantsAccounts.CONTRACT_DATA_CONTRACTINGSPECIALIST_1_SECRETKEY, Constants.USER_FED);
		T1WorkspacePage.goToAccountDetailsPage();
		AccountDetailsPage.goToPageOnSideNav("My Roles");
		...
		boolean userAlreadyHasRole2 = MyRolesPage.userHasRole(Constants.ORG_GSA, Constants.ROLE_ASSISTANCE_USER,
				Constants.DOMAIN_ASSISTANCE_LISTING, Constants.NOACTION);
		Assert.assertEquals(true, userAlreadyHasRole2);

		// --------------------------
		boolean roleHistoryFound = UserDirectoryViewAccessPage.latestRoleHistoryFound("shah raiaan",
				Constants.ROLEHISTORY_STATUS_ROLE_UPDATED, Constants.ROLE_ASSISTANCE_USER,
				Constants.ORG_GSA, Constants.NOACTION);
		Assert.assertEquals(true, roleHistoryFound);
		...
		boolean userAlreadyHasRole3 = MyRolesPage.userHasRole(Constants.ORG_GSA, Constants.ROLE_ASSISTANCE_USER,
				Constants.DOMAIN_ASSISTANCE_LISTING, Constants.DELETE);
		Assert.assertEquals(true, userAlreadyHasRole3);
		LaunchBrowserUtil.delay(5);
